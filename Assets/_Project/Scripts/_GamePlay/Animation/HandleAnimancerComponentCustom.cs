...
public class HandleAnimancerComponentCustom : MonoBehaviour
{
    [SerializeField] private AnimancerComponent animancerComponent;
    public AnimancerComponent AnimancerComponent => animancerComponent;

    public bool IsPlaying(AnimationClip clip) => animancerComponent.IsPlaying(clip);

    public void PlayAnim(AnimationClip clip, Action _endAnim = null, float _durationFade = .2f,
        ...
        FadeMode mode = default)
    {
        ...
        void Handle()
        {
            var state = animancerComponent.Play(clip, clip.length * _durationFade, mode);
            if (_endAnim != null)
            {
